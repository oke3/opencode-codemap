...
function renderBuildCommands(project: ProjectModel): string {
  ...
  if (isJsFamily(lang)) {
    ...
    if (py?.linter) {
      ...
      const install = buildTool === "uv" ? `uv run ${cmd}` : `pipx run ${cmd}`;
      lines.push(`- **Lint**: \`${install} .\` (${py.linter})`);
    }
    if (py?.formatter) {
      const cmd = py.formatter === "ruff" ? "ruff format" : py.formatter;
      const prefix = buildTool === "uv" ? "uv run" : buildTool === "poetry" ? "poetry run" : "";
      lines.push(`- **Format**: \`${prefix} ${cmd} .\` (${py.formatter})`);
    }
    if (py?.testRunner) {
      const prefix = buildTool === "uv" ? "uv run" : buildTool === "poetry" ? "poetry run" : "";
      lines.push(`- **Test**: \`${prefix} ${py.testRunner}\``);
    }
    if (py?.buildTool) {
...
export const agentsGenerator: GeneratorPlugin = {
  ...
  async generate(project: ProjectModel): Promise<GeneratedFile[]> {
    ...
    const content = [
      ...
      "",
      `<!-- Generated by @oke3/opencode-codemap v0.1.1 -->`,
